...
export default function Outputs() {
  ...
  const generateWithAI = async () => {
    ...
    setAiError(null);
    try {
      const { outputs: aiOutputs, model, partial, failures } = await generateOutputsWithAI(project, template);
      // Fill any gaps with deterministic output (partial-result contract).
      let finalOutputs = { ...aiOutputs };
      if (partial) {
        const dna = buildProjectDNA(project);
        const deterministic = generateAll(dna);
        for (const k of Object.keys(deterministic)) {
          if (!finalOutputs[k]) finalOutputs[k] = deterministic[k];
        }
      }
      const updated = updateProject(project.id, {
        generatedOutputs: finalOutputs,
        generatedBy: partial ? "hybrid" : "ai",
      ...
      setProject(updated);
      if (partial) {
        const missed = Object.keys(failures || {}).map(k => k.replace(/Prompt|Spec|Checklist/g, "")).join(", ");
        toast.success(`AI outputs ready · ${missed} used template fallback`);
      } else {
          ...
          <div className="animate-fade-in">
            <div className="pw-eyebrow mb-2 flex items-center gap-2">
              ...
              >
                {generatedBy === "ai" ? (
                  ...
                  <><Sparkles className="w-2.5 h-2.5" /> AI + template (partial)</>
                ) : project.aiFallback ? (
            ...
            <p className="text-fg-muted mt-2 text-[14.5px] max-w-xl">
              {generatedBy === "ai"
                ...
                : generatedBy === "hybrid"
                ? "Most outputs generated by Claude. The rest fell back to the template so you're never blocked."
                : project.aiFallback
